import java.util.Iterator;
import java.util.NoSuchElementException;

/* General version */
public class MyLinkedList<T> implements ListInterface<T> { // ListInterface implements Iterable
	// dummy head
	Node<T> head;
	int numItems;

	public MyLinkedList() {
		head = new Node<T>(null);
	}

	/**
	 * {@code Iterable<T>}를 구현하여 iterator() 메소드를 제공하는 클래스의 인스턴스는
	 * 다음과 같은 자바 for-each 문법의 혜택을 볼 수 있다.
	 *
	 * <pre>
	 *  for (T item: iterable) {
	 *  	item.someMethod();
	 *  }
	 * </pre>
	 *
	 * @see PrintCmd#apply(MovieDB)
	 * @see SearchCmd#apply(MovieDB)
	 * @see java.lang.Iterable#iterator()
	 */
	public final MyLinkedListIterator<T> iterator() {
		return new MyLinkedListIterator<>(this);
	}

	@Override
	public boolean isEmpty() {
		return head.getNext() == null;
	}

	@Override
	public int size() {
		return numItems;
	}

	@Override
	public T first() {
		return head.getNext().getItem();
	}

	@Override
	public void add(T item) {
		Node<T> last = head;
		while (last.getNext() != null) {
			last = last.getNext();
		}
		last.insertNext(item);
		numItems += 1;
	}

	@Override
	public void removeAll() {
		head.setNext(null);
	}
}

class MyLinkedListIterator<T> implements Iterator<T> {
	// FIXME implement this
	// Implement the iterator for MyLinkedList.
	// You have to maintain the current position of the iterator.
	private MyLinkedList<T> list;
	private Node<T> curr;
	private Node<T> prev;

	public MyLinkedListIterator(MyLinkedList<T> list) {
		this.list = list;
		this.curr = list.head;
		this.prev = null;
	}

	@Override
	public boolean hasNext() {
		return curr.getNext() != null;
	}

	@Override
	public T next() {
		if (!hasNext())
			throw new NoSuchElementException();

		prev = curr;
		curr = curr.getNext(); // cuz, there's dummy head

		return curr.getItem();
	}

	@Override
	public void remove() {
		if (prev == null)
			throw new IllegalStateException("next() should be called first");
		if (curr == null)
			throw new NoSuchElementException();
		prev.removeNext();
		list.numItems -= 1;
		curr = prev;
		prev = null;
	}

	public void insert(T newItem) {
		if (prev == null)
			throw new IllegalStateException("next() should be called first");

		prev.insertNext(newItem);
		list.numItems += 1;
		curr = prev;
		prev = null;
	}
}

/* Comparable Version */
//public class MyLinkedList<T extends Comparable<T>> implements ListInterface<T> { // ListInterface implements Iterable
//	// dummy head
//	Node<T> head;
//	int numItems;
//
//	public MyLinkedList() {
//		head = new Node<>(null);
//	}
//
//    /**
//     * {@code Iterable<T>}를 구현하여 iterator() 메소드를 제공하는 클래스의 인스턴스는
//     * 다음과 같은 자바 for-each 문법의 혜택을 볼 수 있다.
//     *
//     * <pre>
//     *  for (T item: iterable) {
//     *  	item.someMethod();
//     *  }
//     * </pre>
//     *
//     * @see PrintCmd#apply(MovieDB)
//     * @see SearchCmd#apply(MovieDB)
//     * @see java.lang.Iterable#iterator()
//     */
//    public final Iterator<T> iterator() {
//    	return new MyLinkedListIterator<>(this);
//    }
//
//	@Override
//	public boolean isEmpty() {
//		return head.getNext() == null;
//	}
//
//	@Override
//	public int size() {
//		return numItems;
//	}
//
//	@Override
//	public T first() {
//		return head.getNext().getItem();
//	}
//
//	@Override
//	public void add(T item) {
//		Node<T> curr = head;
//		int compare = curr.getNext().getItem().compareTo(item);
//
//		while ((curr.getNext() != null) && (compare < 0)) {
//			curr = curr.getNext();
//			compare = curr.getNext().getItem().compareTo(item);
//		}
//
//		// If there already is 'item'
//		if (compare != 0) {
//			curr.insertNext(item);
//			numItems += 1;
//		}
//	}
//
//	@Override
//	public void removeAll() {
//		head.setNext(null);
//	}
//}
//
//class MyLinkedListIterator<T extends Comparable<T>> implements Iterator<T> {
//	// FIXME implement this
//	// Implement the iterator for MyLinkedList.
//	// You have to maintain the current position of the iterator.
//	private MyLinkedList<T> list;
//	private Node<T> curr;
//	private Node<T> prev;
//
//	public MyLinkedListIterator(MyLinkedList<T> list) {
//		this.list = list;
//		this.curr = list.head;
//		this.prev = null;
//	}
//
//	@Override
//	public boolean hasNext() {
//		return curr.getNext() != null;
//	}
//
//	@Override
//	public T next() {
//		if (!hasNext())
//			throw new NoSuchElementException();
//
//		prev = curr;
//		curr = curr.getNext(); // cuz, there's dummy head
//
//		return curr.getItem();
//	}
//
//	@Override
//	public void remove() {
//		if (prev == null)
//			throw new IllegalStateException("next() should be called first");
//		if (curr == null)
//			throw new NoSuchElementException();
//		prev.removeNext();
//		list.numItems -= 1;
//		curr = prev;
//		prev = null;
//	}
//}
